...
class SignUp extends React.Component {
    ...
    handleSubmit = event => {
        const { email, password, error, full_name, graduation, preferred_name} = this.state;

        const { history } = this.props;

        auth.createUser(email, password).then(() => {
            this.setState({ email: "", password: "", error: false });
            db.createUserwithFields(full_name, graduation, preferred_name, email).then(() => {
                this.setState({full_name: "", preferred_name: "", graduation: "", email: ""})
            });
            history.push("/");
        })
        .catch(error => {
